...
Purpose: Current cron logs include npm header noise ("> rondo-sync@0.1.0 sync-all") and running install-cron multiple times creates duplicate entries.
...
This filters out any existing rondo-sync related entries before adding the new ones, making the install idempotent.

The pattern `rondo-sync\|cron-wrapper.sh` catches:
- Comment lines containing "rondo-sync"
...
(Change "installed" to "updated" since this now handles re-installation)
  ...
  <done>install-cron.sh removes existing rondo-sync entries before adding new ones</done>
</task>
