...
4. Add RONDO_ environment variables to the Environment Variables section:
```bash
# Stadion WordPress
RONDO_URL=              # WordPress site URL (https://...)
RONDO_USERNAME=         # WordPress username
RONDO_APP_PASSWORD=     # Application password (from WordPress profile)
RONDO_PERSON_TYPE=      # Custom post type (default: person)
```
  </action>
  <verify>
