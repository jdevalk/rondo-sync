...
5. Export the new functions in module.exports.
  ...
  <verify>
Run three separate verification commands:

1. Verify tables exist:
```bash
node -e "const db = require('./lib/stadion-db'); const d = db.openDb(':memory:'); const tables = d.prepare(\"SELECT name FROM sqlite_master WHERE type='table'\").all().map(t => t.name); const ok = ['stadion_change_detections', 'reverse_sync_state'].every(t => tables.includes(t)); console.log(ok ? 'PASS: Tables created' : 'FAIL: Missing tables'); process.exit(ok ? 0 : 1)"
```

2. Verify tracked_fields_hash column:
```bash
node -e "const db = require('./lib/stadion-db'); const d = db.openDb(':memory:'); const cols = d.prepare('PRAGMA table_info(stadion_members)').all(); const ok = cols.some(c => c.name === 'tracked_fields_hash'); console.log(ok ? 'PASS: tracked_fields_hash column exists' : 'FAIL: Missing column'); process.exit(ok ? 0 : 1)"
```

3. Verify helper functions exported:
```bash
node -e "const db = require('./lib/stadion-db'); const fns = ['logChangeDetection', 'getChangeDetections', 'getLastDetectionTime', 'updateLastDetectionTime']; const missing = fns.filter(f => typeof db[f] !== 'function'); if (missing.length === 0) { console.log('PASS: All helper functions exported'); process.exit(0); } else { console.log('FAIL: Missing exports:', missing.join(', ')); process.exit(1); }"
```

All three commands must output PASS and exit with code 0.
  </verify>
...
5. Implement `fetchModifiedMembers(since, options)`:
   - Use stadionRequest to query Stadion API with modified_after parameter:
     ```javascript
     ...
     ```
   - Follow existing pagination pattern from submit-stadion-teams.js (check response.body.length < 100 to detect last page)
   - The modified_after parameter filters for members modified since the given ISO timestamp
...
Create detect-stadion-changes.js CLI entry point with explicit structure:
...
This follows the same module/CLI hybrid pattern as other scripts (e.g., submit-stadion-teams.js), with:
- Shebang for direct execution
- --verbose/-v flag parsing
- Logger integration
- Error handling with proper exit codes
- Exports main for testing
  </action>
  <verify>
...
1. Verify self-test block exists in the file:
```bash
grep -q "Self-Test" lib/detect-stadion-changes.js && echo "PASS: Self-test block exists" || (echo "FAIL: Self-test block not found" && exit 1)
```

2. Run the self-test:
```bash
node lib/detect-stadion-changes.js 2>&1 | tail -1 | grep -q "All tests passed" && echo "PASS: Self-test passed" || (echo "FAIL: Self-test did not pass" && exit 1)
```

Both commands must output PASS.
  </verify>
