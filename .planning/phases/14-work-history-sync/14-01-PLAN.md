...
6. **runSync(options)** - Main orchestration
   ...
   - Open database (laposta-sync.sqlite for Sportlink data, stadion-sync.sqlite for tracking)
...
3. **Verify SQLite tracking:**
   ...
   - Run: `sqlite3 stadion-sync.sqlite "SELECT knvb_id, team_name, stadion_work_history_id FROM stadion_work_history LIMIT 5"`
...
1. **SQLite schema:** `sqlite3 stadion-sync.sqlite ".schema stadion_work_history"` shows table with stadion_work_history_id column and UNIQUE(knvb_id, team_name)
