...
must_haves:
  ...
  artifacts:
    - path: "lib/stadion-db.js"
      ...
      exports: ["upsertWorkHistory", "getWorkHistoryNeedingSync", "updateWorkHistorySyncState", "getMemberWorkHistory", "getWorkHistoryByMember", "deleteWorkHistory", "deleteAllMemberWorkHistory", "computeWorkHistoryHash"]
  ...
  key_links:
    ...
    - from: "submit-stadion-work-history.js"
      to: "laposta-db getLatestSportlinkResults"
      via: "Load Sportlink members from SQLite"
      pattern: "getLatestSportlinkResults\\(db\\)"
---
...
**Important:** The `stadion_work_history_id` column stores the WordPress work_history row index (0, 1, 2...) for entries created by the sync. This distinguishes sync-created entries from manually created ones in WordPress (which won't have an entry in this table).
...
3. `getWorkHistoryNeedingSync(db, force)` - Get records where hash changed
   ...
   - Return: [{ knvb_id, team_name, is_backfill, source_hash, stadion_id, stadion_work_history_id }]

4. `getMemberWorkHistory(db, knvbId)` - Get all work history for a member
   - Return: [{ team_name, is_backfill, last_synced_at, stadion_work_history_id }]

5. `getWorkHistoryByMember(db)` - Get all work history grouped by member
   - Return Map<knvb_id, Set<team_name>> for change detection

6. `updateWorkHistorySyncState(db, knvbId, teamName, sourceHash, stadionWorkHistoryId)` - Mark as synced
   - Also stores the stadion_work_history_id (row index in WordPress) for future updates
...
Export all new functions in module.exports.
  ...
  <verify>
Run: `node -e "const db = require('./lib/stadion-db'); console.log(Object.keys(db).filter(k => k.includes('WorkHistory')))"` should show all 8 work history functions: computeWorkHistoryHash, upsertWorkHistory, getWorkHistoryNeedingSync, getMemberWorkHistory, getWorkHistoryByMember, updateWorkHistorySyncState, deleteWorkHistory, deleteAllMemberWorkHistory.
  </verify>
  <done>
stadion_work_history table exists with stadion_work_history_id column and composite unique key on (knvb_id, team_name). All 8 work history tracking functions exported and usable.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Create submit-stadion-work-history.js following established patterns from submit-stadion-sync.js and prepare-stadion-teams.js:
...
4. **detectTeamChanges(db, knvbId, currentTeams)** - Compare current vs tracked teams
   - Get tracked teams from getMemberWorkHistory (returns team_name + stadion_work_history_id)
   - Return { added: [], removed: [], unchanged: [] }
   - removed array includes stadion_work_history_id for each removed team
...
5. **syncWorkHistoryForMember(member, currentTeams, db, teamMap, options)** - Sync one member
   ...
   - For removed teams (ONLY sync-created, identified by having stadion_work_history_id):
     - End the work_history entry (set end_date, is_current=false)
     - Delete from SQLite tracking (deleteWorkHistory)
   - Fetch existing work_history from Stadion (GET /wp/v2/people/{id})
   - Merge: keep manually created entries (those not tracked in SQLite) + update sync entries
   - PUT updated work_history array to Stadion
   - Update SQLite tracking with stadion_work_history_id for new entries

6. **runSync(options)** - Main orchestration
   - options: { logger, verbose, force, backfillOnly }
   - Open database (laposta-sync.sqlite for Sportlink data, rondo-sync.sqlite for tracking)
   - Load team mapping: getAllTeams(db) -> Map<team_name, stadion_id>
   - Load Sportlink members using getLatestSportlinkResults(db) from laposta-db, parse Members array
   - For each member:
...
- Member had teams but now has none: end ONLY sync-created work_history entries (those tracked in stadion_work_history table with stadion_work_history_id). Manually created entries in WordPress (not in our tracking table) are preserved.
- Team not found in Stadion: log warning, skip that team
- Preserve manually created entries: when fetching existing work_history from Stadion, only modify entries at indices we track (stadion_work_history_id column). Entries at other indices are manual.
...
Run: `node submit-stadion-work-history.js --verbose` should show work history sync progress and results (may show 0 if already synced).
  ...
  <done>
submit-stadion-work-history.js creates work_history entries in Stadion linking persons to teams. Members can have multiple teams. Team changes are detected and only sync-created entries are ended (manual entries preserved). stadion_work_history_id column tracks which WordPress row index belongs to which sync entry.
  </done>
</task>
...
3. **Verify SQLite tracking:**
   - Run: `sqlite3 rondo-sync.sqlite "SELECT COUNT(*) FROM stadion_work_history"`
   - Should show count matching members with teams
   - Run: `sqlite3 rondo-sync.sqlite "SELECT knvb_id, team_name, stadion_work_history_id FROM stadion_work_history LIMIT 5"`
   - Verify stadion_work_history_id is populated for synced entries

4. **Inspect sample data:**
   - Run: `sqlite3 rondo-sync.sqlite "SELECT * FROM stadion_work_history LIMIT 5"`
   - Verify: Records have knvb_id, team_name, stadion_work_history_id, source_hash, last_synced_hash

If any issues found, fix them in the sync script before completing.
  </action>
  <verify>
Work history sync runs without errors. SQLite has tracking records with stadion_work_history_id populated. Stadion persons have work_history entries with correct team references.
  </verify>
  <done>
Work history sync creates backfilled entries correctly. Hash-based change detection prevents duplicate syncs. SQLite tracks all member-team pairings with stadion_work_history_id for update tracking.
  </done>
</task>
...
1. **SQLite schema:** `sqlite3 rondo-sync.sqlite ".schema stadion_work_history"` shows table with stadion_work_history_id column and UNIQUE(knvb_id, team_name)
...
4. **Manual entries preserved:** If a person has manually created work_history in WordPress, verify those entries are NOT modified by sync

5. **No regressions:** `npm run sync-all` still works (Phase 15 will integrate work history)
</verification>

<success_criteria>
- [ ] stadion_work_history table exists with stadion_work_history_id column and proper schema
- [ ] All 8 work history functions exported from lib/stadion-db.js
- [ ] submit-stadion-work-history.js exports runSync and works as CLI
- [ ] Uses getLatestSportlinkResults to load Sportlink members (not getAllMembers)
...
- [ ] Only sync-created entries are ended (tracked via stadion_work_history_id)
- [ ] Manually created work_history in WordPress preserved (not overwritten)
- [ ] SQLite tracks all member-team pairings with stadion_work_history_id
</success_criteria>
