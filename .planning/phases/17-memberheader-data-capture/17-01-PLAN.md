...
```bash
ssh root@46.202.155.16 "cd /home/sportlink && sqlite3 stadion-sync.sqlite 'SELECT knvb_id, has_financial_block, photo_url, photo_date FROM sportlink_member_free_fields LIMIT 5;'"
```
  </verify>
  <done>
...
```bash
ssh root@46.202.155.16 "cd /home/sportlink && sqlite3 stadion-sync.sqlite 'SELECT COUNT(*) as total, SUM(has_financial_block) as with_block, SUM(CASE WHEN photo_url IS NOT NULL THEN 1 ELSE 0 END) as with_photo FROM sportlink_member_free_fields;'"
```
