...
user_setup:
  - service: stadion
    ...
    env_vars:
      ...
      - name: STADION_USERNAME
        source: "WordPress username with REST API access"
      - name: STADION_APP_PASSWORD
...
must_haves:
  ...
  key_links:
    - from: "lib/stadion-client.js"
      to: "process.env.STADION_*"
      via: "varlock/auto-load and readEnv()"
      pattern: "process\\.env\\.STADION_"
    - from: "lib/stadion-client.js"
...
1. **Environment loading** (top of file):
   ...
   - Load STADION_URL, STADION_USERNAME, STADION_APP_PASSWORD
...
2. **Core request function** `stadionRequest(endpoint, method, body = null)`:
   ...
   - Parse STADION_URL to extract hostname
...
3. **Credential validation** (called at start of request):
   ...
   - Check STADION_URL starts with 'https://'
   - Throw clear error if missing: "STADION_URL, STADION_USERNAME, and STADION_APP_PASSWORD required in .env"
...
3. **CLI runs without env vars (should fail gracefully):**
   ```bash
   STADION_URL= STADION_USERNAME= STADION_APP_PASSWORD= node lib/stadion-client.js 2>&1 | head -1
