...
3. testConnection(options) function:
   - First check if FREESCOUT_API_KEY and FREESCOUT_BASE_URL are set
   - If missing: return { success: false, error: 'Missing FREESCOUT_API_KEY and/or FREESCOUT_BASE_URL' }
   - If set: GET /api/users/me to verify credentials
...
4. Add CLI entry point:
   ...
   - Without env vars: output "Missing FREESCOUT_API_KEY and/or FREESCOUT_BASE_URL" and exit 1
   - With valid env vars: output "FreeScout connection OK" and exit 0
   - With invalid credentials: output error message and exit 1

Use require('varlock/auto-load') at top for env loading.
  </action>
  <verify>
Without FREESCOUT_API_KEY/FREESCOUT_BASE_URL in .env: `node lib/freescout-client.js` outputs error about missing env vars and exits 1.
With valid FREESCOUT_API_KEY and FREESCOUT_BASE_URL in .env: `node lib/freescout-client.js --verbose` outputs "FreeScout connection OK" and exits 0.
  </verify>
  <done>
freescout-client.js exports freescoutRequest and testConnection, CLI works for connection testing with clear error messages for missing configuration.
  </done>
</task>
...
4. Without env vars: `node lib/freescout-client.js` exits 1 with clear message about missing FREESCOUT_API_KEY and FREESCOUT_BASE_URL
5. With env vars set: `node lib/freescout-client.js` succeeds with exit 0
</verification>
...
- API client shows clear error when env vars missing (exit 1)
- API client can authenticate with FreeScout when configured (exit 0)
