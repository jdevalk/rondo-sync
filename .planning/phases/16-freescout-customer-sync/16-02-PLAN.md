...
must_haves:
  ...
  key_links:
    ...
    - from: "prepare-freescout-customers.js"
      ...
      via: "nikki-db getContributionsByKnvbId (optional, graceful null on missing)"
    ...
    - from: "submit-freescout-sync.js"
      ...
      via: "freescout-db for authoritative freescout_id tracking"
      pattern: "freescout-db"
    - from: "sync-all.js"
...
1. Load data from multiple sources:
   ...
   - nikki_contributions table (lib/nikki-db.js) - most recent year's saldo/status
     IMPORTANT: Nikki data is OPTIONAL. If nikki-sync.sqlite doesn't exist or has no data
     for a member, use null values for nikki_saldo and nikki_status. Do NOT fail or skip
     members if Nikki data is unavailable.
   - sportlink_member_free_fields table (lib/stadion-db.js) - existing FreeScout ID (secondary source)

2. FreeScout ID resolution (for initial sync):
   - Check freescout_customers table first (authoritative source from our tracking)
   - Fall back to sportlink_member_free_fields.freescout_id (user may have manually entered)
   - This is for seeding only; after sync, freescout_customers is the authoritative source

3. Transform each member to FreeScout customer format:
   ```javascript
   {
     knvb_id: member.knvb_id,
     email: member.email,
     freescout_id: getExistingFreescoutId(member.knvb_id),  // From our tracking DB or Sportlink
     data: {
     ...
     customFields: {
       ...
       nikki_saldo: getMostRecentNikkiSaldo(member.knvb_id),  // null if no Nikki data
       nikki_status: getMostRecentNikkiStatus(member.knvb_id)  // null if no Nikki data
     }
   }
   ```

4. Helper functions:
   - getExistingFreescoutId(knvbId): Check freescout_customers first, then sportlink_member_free_fields
   ...
   - getMostRecentNikkiSaldo(knvbId): Query nikki_contributions for most recent year, return null if not found
   - getMostRecentNikkiStatus(knvbId): Query nikki_contributions for most recent year, return null if not found

5. Nikki database handling:
   - Try to open nikki-sync.sqlite, but if it doesn't exist, continue without error
   - If Nikki DB is unavailable, all Nikki fields will be null (this is acceptable)
   - Log a warning once if Nikki database is not available

6. Filter: Only include members with valid email addresses.

7. Export runPrepare(options) returning { success: true, customers: [...] } or { success: false, error: '...' }
...
8. CLI entry point:
  ...
  <verify>
Run: `node prepare-freescout-customers.js` outputs "Prepared X customers for FreeScout" with X > 0 (even if Nikki data is not available).
  </verify>
  <done>
prepare-freescout-customers.js transforms member data to FreeScout format, including custom fields from multiple data sources, with graceful handling of missing Nikki data (null values).
  </done>
</task>
...
1. Main sync flow:
   ...
   - Upsert to freescout_customers tracking table (this is the AUTHORITATIVE source for freescout_id)
   - Get customers needing sync (hash changed)
   - For each customer:
     a. If freescout_id exists in our tracking DB: PUT /api/customers/{id}
     b. If no freescout_id: GET /api/customers?email=... to search
     c. If found by email: Update existing customer and store freescout_id in our DB
     d. If not found: POST /api/customers to create, store new freescout_id in our DB
     e. After success: PUT /api/customers/{id}/customer_fields for custom fields
     f. Update tracking database with freescout_id and hash

2. FreeScout ID storage architecture:
   - AUTHORITATIVE: freescout_customers.freescout_id (our SQLite tracking database)
   - SECONDARY: sportlink_member_free_fields.freescout_id (read-only, for initial seeding)
   - We do NOT write freescout_id back to Sportlink - we track it in our own database
   - After first successful sync, freescout_customers table has the mapping

3. Custom fields format (per FreeScout API):
   ```javascript
   const customFields = [
...
4. Error handling:
   - Handle 404 on update (customer deleted): clear freescout_id in our DB, create new
...
5. Export runSubmit(options) returning:
   ```javascript
   {
     success: boolean,
     total: number,
...
Run: `node submit-freescout-sync.js --dry-run --verbose` shows customers that would be synced without making API calls.
  ...
  <done>
submit-freescout-sync.js syncs customers to FreeScout with hash-based change detection, search-before-create logic, and custom field updates. FreeScout IDs are tracked in freescout_customers table (our authoritative source), not written back to Sportlink.
  </done>
</task>
...
5. Ensure FreeScout sync runs AFTER:
   ...
   Note: Nikki sync runs separately (sync-nikki.js) and may or may not have populated
   nikki_contributions. FreeScout sync handles missing Nikki data gracefully with null values.
  </action>
  <verify>
Run: `node sync-all.js --verbose 2>&1 | grep -i freescout` shows FreeScout sync output in pipeline.
  </verify>
  <done>
sync-all.js includes FreeScout sync as final step after members, photos, and teams are synced. Nikki data is optional and handled gracefully if not present.
  </done>
</task>
...
<verification>
1. `node prepare-freescout-customers.js` outputs customer count > 0 (even without Nikki data)
...
6. Nikki fields show as empty if Nikki data not available (not an error)
</verification>

<success_criteria>
- prepare-freescout-customers.js transforms member data from all sources
- Missing Nikki data results in null values, not errors or skipped members
- submit-freescout-sync.js syncs to FreeScout with hash-based change detection
- FreeScout IDs tracked in freescout_customers table (not written to Sportlink)
- Custom fields populated: KNVB ID, teams, member_since, Nikki saldo/status (or null)
