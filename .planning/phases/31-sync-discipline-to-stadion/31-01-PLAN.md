...
1. `buildPersonLookup()` - Load knvb_id -> stadion_id mapping from rondo-sync.sqlite
   - Open rondo-sync.sqlite (not discipline-sync.sqlite)
...
- Person lookup loads from rondo-sync.sqlite
...
5. Person linking uses stadion_id from rondo-sync.sqlite lookup
