...
<task type="auto">
  ...
  <action>
Add `photos/` to `.gitignore` under the Data section (after `rondo-sync.sqlite`).
...
3. **Check database state:**
   ```bash
   sqlite3 rondo-sync.sqlite "SELECT photo_state, COUNT(*) FROM stadion_members GROUP BY photo_state"
