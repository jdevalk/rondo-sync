...
3. Export all new functions in module.exports (including getAllTrackedMembers)
  </action>
  <verify>
Run: `node -e "const db = require('./lib/stadion-db'); console.log(Object.keys(db).filter(k => k.includes('Parent') || k.includes('AllTracked')))"`
Expected: Array containing computeParentHash, upsertParents, getParentsNeedingSync, updateParentSyncState, deleteParent, getParentsNotInList, getAllTrackedMembers
  </verify>
  <done>
stadion-db.js has parent tracking table, all parent-related functions, and getAllTrackedMembers exported
  </done>
</task>
