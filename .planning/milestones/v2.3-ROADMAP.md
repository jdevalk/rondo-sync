...
**Success Criteria** (what must be TRUE):
  ...
  3. The `stadion_important_dates` table no longer exists (or is fully unused) in `rondo-sync.sqlite`
...
**Plan 33-01** added `extractBirthdate()` function to `prepare-stadion-members.js` and removed the birthday sync step from the people pipeline (8 steps → 7 steps). Birthday data now flows through the existing person sync with hash-based change detection. Commits: `1e075be`, `6c36fd4`.
