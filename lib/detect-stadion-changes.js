  ...
  try {
    ...
    for (const member of modifiedMembers) {
      ...
      for (const field of TRACKED_FIELDS) {
        ...
        const parsedOldData = oldData ? JSON.parse(oldData.data_json) : {};

        // Extract old value using same extraction logic
        ...
        // For now, we'll log the change without old value comparison
        // (old values would require fetching previous Stadion state)
        const change = {
          knvb_id: knvbId,
          field_name: field,
          old_value: null, // Will be populated by reverse sync when it fetches current state
        ...
        logger.verbose(`  - ${field}: changed to ${newValue}`);
      }
    }
