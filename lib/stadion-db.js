...
function upsertCommissieWorkHistory(db, records) {
  ...
  const stmt = db.prepare(`
    ...
    ON CONFLICT(knvb_id, commissie_name) DO UPDATE SET
      source_hash = excluded.source_hash,
      is_backfill = excluded.is_backfill
  `);
...
function getMemberCommissieWorkHistory(db, knvbId) {
  const stmt = db.prepare(`
    SELECT commissie_name, is_backfill, last_synced_at, stadion_work_history_id
 ...
 */
function updateCommissieWorkHistorySyncState(db, knvbId, commissieName, sourceHash, stadionWorkHistoryId) {
  const now = new Date().toISOString();
  const stmt = db.prepare(`
    UPDATE stadion_commissie_work_history
    SET last_synced_at = ?, last_synced_hash = ?, stadion_work_history_id = ?
    WHERE knvb_id = ? AND commissie_name = ?
  `);
  stmt.run(now, sourceHash, stadionWorkHistoryId, knvbId, commissieName);
}

/**
 * Delete a commissie work history record from tracking.
 */
function deleteCommissieWorkHistory(db, knvbId, commissieName) {
  const stmt = db.prepare(`
    DELETE FROM stadion_commissie_work_history
    WHERE knvb_id = ? AND commissie_name = ?
  `);
  stmt.run(knvbId, commissieName);
}
